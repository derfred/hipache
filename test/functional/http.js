(function () {
    ...
    var host = function (name, options) {
        if(!options) {
            options = {};
        }
        ...
        return new (function () {
            ...
            this.queries = function (method, stream, statusCode) {
                ...
                it(testName, function (done) {
                    var opts = {
                        hostname: options.hostname || hipacheHost,
                        headers: {Host: name},
                        port: options.port || hipachePort,
                        path: '',
                        method: method || 'GET'
                    };
    ...
    describe('with default test config', function() {
        // Start hipache
        before(function (done) {
    ...
    describe('with custom logger', function(done) {
        before(function (done) {
            fs.unlink('/tmp/my_hipache_logger.log', function() {
                commander.hipache.start('test/fixtures/configs/hipache-config-logger.json', done);
            });
        });

        host('simple', { port: 2081 }).connects('http://localhost:8201');
        host('simple', { port: 2081 }).gets(201);
        host.flush();

        it('writes info to log file', function(done) {
            var timeCallback = function(tries) {
                if(fs.existsSync('/tmp/my_hipache_logger.log')) {
                    fs.readFile('/tmp/my_hipache_logger.log', "utf8", function (err, data) {
                        if (err) throw err;
                        console.log(data);
                        var log = JSON.parse(data);
                        expect(log.method).to.eql('GET')
                        expect(log.url).to.eql('/')
                        expect(log.statusCode).to.eql(201)
                        expect(log.virtualHost).to.eql('simple')
                        done();
                    });
                } else if(tries > 0) {
                    setTimeout(function() {
                        timeCallback(tries-1);
                    }, 100);
                }
            };
            timeCallback(10);
